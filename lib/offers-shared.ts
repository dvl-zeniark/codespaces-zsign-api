...
/**
 * Client-safe: types + pure helpers only, no import of lib/zsign (undici).
 * Client components (e.g. ApiWorkspace) must import from here, not from
 * lib/offers - that file pulls in undici, which webpack's client bundle
 * can't parse (private-field syntax) once a client component reaches it.
 */
