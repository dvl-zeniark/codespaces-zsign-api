...
/** Astro has no next/navigation - read the query string once on mount, and
 * push updates via the History API directly (no client router here). */
...
export function ApiWorkspace() {
  ...
  const loadSurface = useCallback(
    (next: Surface, nextRequestId?: string, nextDocId?: string) => {
      ...
      replaceQuery((params) => {
        ...
        if (nextDocId) params.set("documentId", nextDocId);
  ...
  useEffect(() => {
    ...
    // Only meant to seed initial state from the URL once on mount.
